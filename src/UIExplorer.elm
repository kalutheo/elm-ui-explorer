...
import Html.Attributes exposing (class, classList, href, id, rel, src, style)
...
{-| Snapshot
-}
viewSnapshot : List ( String, a ) -> (a -> Html msg) -> Html msg
viewSnapshot stories view =
    div []
        (stories
            |> List.map
                (\( name, model ) ->
                    div [ id name, class "snapshot" ] [ view model ]
                )
        )
